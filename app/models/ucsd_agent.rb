...
class UcsdAgent < Agent
  ...
  def to_solr(solr_doc = {})
    super.tap do |doc|
      doc[Solrizer.solr_name('agent_type', :stored_searchable)] = agent_type
      doc[Solrizer.solr_name('orcid', :stored_searchable)] = orcid

      index_authorities doc, :close_match, close_match
      index_authorities doc, :related_match, related_match
      index_authorities doc, :different_from, different_from
    end
  end
end
