...
describe 'TimeSpanInput', type: :input do
  ...
  context 'with time span' do
    ...
    it 'renders time span' do
      ...
      expect(subject).to have_selector('.form-group.foo_bar.time_span ul.listing li div.row div.col-md-1 label', count: 3)
      expect(subject).to have_selector('.form-group.foo_bar.time_span ul.listing li div.row div.col-md-5 input.foo_bar', count: 3)
    end
  end
  ...
  describe '#build_field' do
    ...
    subject { TimeSpanInput.new(builder, :bar, nil, :time_span, {}) }
