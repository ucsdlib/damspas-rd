...
describe 'RelatedResourceInput', type: :input do
  ...
  context 'with related_resource' do
    ...
    it 'renders related_resource' do
      expect(subject).to have_selector('.form-group.foo_relation.multi_value label.required[for=foo_relation]', text: 'Relation')
      expect(subject).to have_selector('.form-group.foo_relation.multi_value ul.listing li div.row div.col-md-1 label', count: 3)
      expect(subject).to have_selector('.form-group.foo_relation.multi_value ul.listing li div.row div.col-md-5 input.foo_relation', count: 2)
    end
  end
  ...
  describe '#build_field' do
    let(:foo) { Foo.new }
    before { foo.relation_attributes = [{related_type: ['relation'], name: ['Related resource name'], url: ['http://test.com/any/url']}] }
    let(:builder) { double('builder', object: foo, object_name: 'foo') }

    subject { RelatedResourceInput.new(builder, :relation, nil, :intangible, {}) }
